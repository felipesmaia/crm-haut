      ...
      {(provided, snapshot) => {
        ...
        return (
            <div 
                ...
                className={`relative bg-white p-4 rounded-xl border cursor-grab hover:shadow-md transition-all group ${
                    isSelected ? 'ring-2 ring-chronos-500 border-chronos-500 bg-chronos-50' : ''
                } ${
                    snapshot.isDragging 
            ...
            >
                ...
                {lastNote && (
                    <div className="absolute inset-x-0 bottom-0 translate-y-[95%] opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none px-2">
                        <div className="bg-gray-800 text-white text-xs p-2.5 rounded-lg shadow-xl border border-gray-700 relative">
                            <div className="absolute -top-1.5 left-8 w-3 h-3 bg-gray-800 rotate-45 border-t border-l border-gray-700"></div>
                            <p className="line-clamp-3 font-medium text-gray-200">"{lastNote.content}"</p>
                            <p className="text-[10px] text-gray-400 mt-1">
                                {lastNote.createdAt} • {lastNote.author}
                            </p>
                        </div>
                    </div>
                )}
                ...
                >
                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${isSelected ? 'bg-chronos-600 border-chronos-600' : 'bg-white border-gray-300 hover:border-chronos-500'}`}>
                ...
                <div className="flex items-start justify-between mb-3 pl-6"> 
                    <div className="flex items-center space-x-3">
                        <img src={lead.avatar} alt={lead.name} className="w-10 h-10 rounded-full object-cover pointer-events-none" />
                        <div>
                ...
                <div className="mb-3">
                    ...
                    <div className="flex items-center justify-between mt-1">
                        ...
                        {lead.notes.length > 0 && (
                            <div className="flex items-center text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full" title={`${lead.notes.length} notas`}>
                ...
                <div className="flex flex-wrap gap-2">
                    <span className={`text-[10px] px-2 py-1 rounded-md font-bold border uppercase tracking-wider ${getStatusColor(lead.status)}`}>
                        {lead.status}
                    </span>

                    {lead.tags.map((tag, idx) => (
                        <span key={idx} className={`text-[10px] px-2 py-1 rounded-md font-medium border
                            ${tag === 'Alta Intenção' ? 'bg-yellow-50 text-yellow-700 border-yellow-100' : 
